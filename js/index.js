...
const renderFeedbackMessage = (isDelete = false) => {
  const [popup] = document.getElementsByClassName('feedback-message')
  popup.classList.add('feedback-message-active')
  // const messageText = document.getElementById('feedback-message-text')

  const message = isDelete ? 'Item removido do carrinho com sucesso!' : 'Item adicionado no carrinho com sucesso!'
  popup.innerHTML = ` <i style="color: green" class="fa-regular fa-circle-check"></i> ${message}`

  setTimeout(() => {
    popup.classList.remove('feedback-message-active')
  }, 3000)
}
